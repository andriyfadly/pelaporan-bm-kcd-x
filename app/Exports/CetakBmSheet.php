...
class CetakBmSheet implements FromCollection, WithEvents, WithTitle
{
    ...
    public function collection(): Collection
    {
        ...
        foreach ($this->records as $row) {
            ...
            $nilai = (float) ($row->nilai_perolehan ?? ($volume * $harga));
            $rows->push([
                $no++,
                $row->no_sp2d ?? '',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('A8:Z9')->applyFromArray([
                    'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'DDEBF7']],
                    'font' => ['size' => 11, 'name' => 'Calibri'],
                    'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER, 'wrapText' => true],
                    'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                foreach (['C8', 'D8', 'E8', 'F8', 'F9', 'G9', 'H9', 'I9', 'J8', 'L9', 'M9', 'N9', 'O9', 'P9', 'Q9', 'Y8', 'Z8'] as $cell) {
                    $sheet->getStyle($cell)->getFont()->getColor()->setRGB('FF0000');
                }

                if ($lastRow >= 10) {
                    $sheet->getStyle("A10:Z{$lastRow}")->getFont()->setSize(11)->setName('Calibri');
                    $sheet->getStyle("A10:Z{$lastRow}")->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->getColor()->setRGB('000000');
                ...
                foreach (range('B', 'Z') as $col) {
                    $sheet->getColumnDimension($col)->setWidth(in_array($col, ['B', 'E', 'K', 'L', 'M', 'T', 'Y', 'Z']) ? 16 : 14);
                }
