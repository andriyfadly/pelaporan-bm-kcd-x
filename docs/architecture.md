...
3. **`Spj` (`pelaporan_bm_spj`)**:
   ...
   - Kolom: `no_spk`, `no_sp2d`, `sumber_perolehan`, `bulan_realisasi` (integer), `kategori`, `ba_no`, `ba_tgl`, `kode_barang`, `nama_barang`, `jenis_aset`, `volume`, `harga_satuan`, `nilai_perolehan`, `acuan_id`, `sekolah_id`.
   - Status realisasi TIDAK disimpan sebagai kolom flag — di-derive dari keberadaan row `Realisasi` (`whereHas('realisasi')`, accessor `is_realisasi`, didukung props `realisasiIds`/`teralokasiIds` di Inertia).
4. **`Realisasi` (`pelaporan_bm_realisasi`)**:
   ...
   - Kolom: `spj_id`, `sekolah_id`, `acuan_id`, `kodering_belanja`, `bulan_realisasi` (integer), salinan field dokumen & barang (SP2D, SPK, BA, kode/nama barang, merk, sertifikat, ukuran, satuan, volume, harga, nilai perolehan), `id_realisasi_lama`.
...
5. **`Acuan` (`pelaporan_bm_acuan`)**:
   ...
   - Kolom: `sekolah_id`, `tanggal`, `kodering`, `bku`, `uraian`, `nominal`, `bulan` (integer).
...
7. **`KunciLaporan` (`pelaporan_bm_kunci_laporan`)**:
   ...
   - Kolom `bulan` integer; constraint unique `(sekolah_id, bulan)` — satu baris status per sekolah per bulan.
