...
- **`GET /pelaporan-bm/spj`**
  ...
  - Return: Inertia `PelaporanBm/Spj/Index` (Katalog dokumen SPJ belanja modal identik legacy).
...
- **`POST /pelaporan-bm/spj/store-spk`**
  - Payload: `no_spk`, `no_sp2d`, `sumber_perolehan`, `bulan_realisasi`, `kategori`, `ba_no`, `ba_tgl`, `items: [{kode_barang, nama_barang, jenis_aset, merk_tipe, no_sertifikat, ukuran_bangunan, satuan, volume, harga_satuan}]`.
  - Result: Transaksi atomik simpan/update SPK beserta multi-item.
...
- **`POST /pelaporan-bm/input-realisasi/kirim-laporan`**
  - Payload: `bulan` (1-12).
  - Result: Memvalidasi seluruh kekurangan acuan = 0, lalu mengunci status laporan menjadi `menunggu_approval`.
